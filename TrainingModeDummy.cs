...
public class TrainingModeDummy : MonoBehaviour
{
    ...
    private static List<CommandRecordingDriver.InputChange>  _nextInputs = new List<CommandRecordingDriver.InputChange>();
    ...
    private void Update()
    {
        ...
        if (Ready)
        {
            if (_ExTriggered && DummyRecorder.isPlaying)
            {
        ...
        if (Keyboard.current.f8Key.isPressed)
        {
            ...
            DummyRecorder.inputs.Add(new CommandRecordingDriver.InputChange(_startingNumber + 12, 272));
